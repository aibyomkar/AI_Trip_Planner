...
#     user_input = st.text_area('Share your travel aspirations...', height=140, placeholder="Eg: Plan a 5-day Goa trip for 5 people, prefer beaches & mountains, ₹30K/person budget, forecast next week's 5-day weather & per-day expense.")
...
# streamlit frontend (updated to handle concierge 503s + retry + fallback)
...
# Helper: POST with retries (for 503 / transient network errors)
def post_with_retries(url, payload, attempts=3, base_delay=1, timeout=30):
    """Attempt POST up to `attempts` times on network errors or 5xx responses.
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            # Retry on server error 5xx (including 503)
            if 500 <= resp.status_code < 600:
                last_exc = requests.HTTPError(f"Server returned {resp.status_code}")
                st.write(f"Attempt {attempt} — received {resp.status_code} from server. Retrying...")
                if attempt == attempts:
                    return resp
            ...
            last_exc = e
            st.write(f"Attempt {attempt} — network error: {e}. Retrying...")
            if attempt == attempts:
                raise
            sleep_for = base_delay * (2 ** (attempt - 1))
            time.sleep(sleep_for)
...
if submit_button and user_input.strip():
    try:
        with st.spinner('🔮 Crafting your bespoke itinerary...'):
            # Use retry wrapper to handle transient 503s
            try:
                response = post_with_retries(f'{BASE_URL}/query', {'query': user_input}, attempts=3, base_delay=1, timeout=30)
            except Exception as e:
                response = None
                last_exception = e
...
"""
            ...
            # If response is None (all retries failed) or we received a 5xx
            st.warning('⚠️ Concierge service temporarily unavailable — the AI engine may be overloaded.')
            ...
            with col2:
                if st.button('Get quick fallback plan'):
                    # call fallback endpoint which should return a minimal plan (implement on backend)
                    try:
                        with st.spinner('Generating quick fallback plan...'):
                            fb = requests.post(f'{BASE_URL}/fallback_plan', json={'prompt': user_input}, timeout=30)

                        if fb.status_code == 200:
                            fb_data = fb.json()
                            st.info('— Quick fallback plan —')
                            # if backend returns structured JSON show it, else show raw text
                            if isinstance(fb_data, dict) and fb_data.get('plan'):
                                st.markdown(fb_data.get('plan'))
                            else:
                                st.markdown(fb.text)
                        else:
                            st.error('Fallback generation failed. Please try again later.')
                    except Exception as e:
